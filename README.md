...
| 版本 | 内容 |
|---|---|
| 1.7.14（未升版） | **播放页进度条触摸滑动 seek（触摸屏补 pointer 通道）**：官方 ArtPlayer 进度条拖动只在桌面绑 mouse 事件（minified 源码 `p||(...)` 分支，移动端不绑）、触摸屏滑动被 touch-action 默认滚动接管——表现为「点进度条可以、滑动没反应」。`play-enhance.js` 新增 `initProgressTouch`：给官方 `.art-progress` 设 `touch-action:none` + pointer 事件（仅 `pointerType≠mouse` 介入），按下即跳 + 按住滑动连续 seek + 指针捕获保证拖出进度条不丢尾段；桌面鼠标仍走官方拖动互不干扰；与画面横滑 seek（initDragSeek）/竖滑音量方向分发不冲突。模板 `_iwara-style/public/play-enhance.js` 已同步 |
| 1.7.13（未升版） | **播放页交互（①点赞/关注 toggle ②搜索列表已赞 json 字段 + mtime 局部刷新 ③画面竖滑音量 + 全触摸屏适配 ④进度条图片预览 VTT 插件）**——①播放页点赞/关注按钮补齐取消能力（toggle）+ 措辞「收藏→点赞」：后端新增 `unfollowUser`（`DELETE /user/{id}/followers`）+ `like-state.markUnfollowed` + 路由 `DELETE /api/follow?userId=`；前端 `bindStateButtons` 改 toggle（按 `btn.dataset.on` 判断 DELETE/POST，成功才翻转）。②搜索列表——badge 渲染改**只读搜索 json 的 `liked` 字段**（单一数据源）；`/api/liked-state` 返回 `mtime`（liked_state.json stat.mtimeMs）；轮询改 **mtime 变化才局部更新**：`updateLikedBadges()` 只增删变化行的 badge（`data-vid` 定位），**不整表重建**（滚动不跳、图片不闪）；结构变化（分页）才整表渲染；`/api/search/cache` 返回前也补 merge。③播放器画面手势（**桌面 + 触摸屏全适配**）——上下滑动实时音量（方向分发：首个位移超 12px 判定横/竖并锁定，锁定后不切换；竖滑调 `art.volume` + HUD「音量 N%」、满半屏滑满、钳制 0-1、静音自动解除、控制条区域不触发；桌面 pointer + 移动端 touch 双通道，`touch-action:none` 防滚动接管；与 600ms 长按快进互斥——过阈值即 clearHold + 取消点击即播）；左右滑进度连续 seek（`initDragSeek` 已开放到移动端，触摸横滑同样生效；移动端长按快进保持官方 fastForward 不重复挂自绘）。④进度条图片预览——`artplayer-plugin-vtt-thumbnail` v1.1.0 本地化到 `public/vendor/`（官方插件，UMD，零 npm 依赖，NOTICE.md 记录来源）；服务端 ffmpeg 生成雪碧图 `<id>-thumb.jpg`（N 帧 tile 拼图，N=clamp(round(时长/30),4,16)、≤5s=1 帧、帧宽 160、`-f mjpeg` 显式 muxer）与 VTT `<id>-thumb.vtt`（`#xywh=` 坐标与 tile 布局同参生成不漂移、相对 sprite URL）；新路由 `GET /api/thumbnail-sprite` + `GET /api/thumbnail-vtt`（未生成 404、缓存 max-age 86400）；play-info 完整视频附 `thumbVtt`（后台生成不阻塞，partial/未生成为空）；play.html 引入插件脚本、play-app.js `art.once("ready")` 注册（ArtPlayer 不支持初始化后改 thumbnails 配置，插件化注册天然规避；触摸屏插件内置移动端行为——播放/拖动进度条时显示当前帧，无需 hover）；listCached 过滤 `-thumb.jpg`/`-thumb.vtt`（不参与封面索引/清理）。⑤`assemble.json` 补三条下发条目：`_iwara-style/server/lib/iwara-api.js`、`like-state.js`、`server/routes/like.js` 进模板。验证：jsdom 局部 badge 更新 8/8、竖滑音量 10/10、移动端触摸 8/8、VTT 注册 4/4、播放页 toggle 9/9；extractSprite 实测 N=1/N=4 tile 生成 + VTT 坐标正确；两接口 404/200 实测；真实环境搜索 task.results 每条含 liked（96 条）、`/api/liked-state` 含 mtime、官方 `POST/DELETE /user/{id}/followers` 200。模板 `play-app.js`/`play.html`/`app.js`/`play-enhance.js`/`vendor/`（含插件 + NOTICE.md）/`server/lib/*`/`server/routes/like.js` 已同步。**sprite 生成修复（2026-09-20 实测）**——真实环境发现预览图不显示，根因两个：①`tool/ffmpeg`/`tool/ffprobe` wrapper 在 git 里权限是 100644（无执行位），auto-update checkout 落盘后 spawn EACCES、生成静默失败（封面走官方网络拉取所以不受影响）→ 已 `git update-index --chmod=+x` 修正 git 模式；②`extractSprite` 原用 `fps=1/interval` 滤镜需全片解码（643s 视频 60s 超时被杀）→ 改 **`-ss` 输入 seek 分帧抽帧（单帧实测 0.2~0.3s）+ 二次 ffmpeg concat+tile 拼图**，VTT 时间/坐标同参生成。同时接入**模板统一工具探测 `server/tool/tool-detect.js`**（框架层新模块：环境变量→项目 tool/ 与 tools/→系统路径，每级「存在+可执行+版本」实测、失败自动降级、带缓存）；`findFfmpeg`/`findFprobe` 改走 `detectTool`（`findFfmpegLib` 取 wrapper 随行 lib 注入 `LD_LIBRARY_PATH`），projectRoot 改为**逐级向上探测项目根标志**而非固定级数（避免组装后位置变化算错）；spawn 失败/ffmpeg 错误日志落 console（`[thumb] sprite:` 前缀）。验证：N=1/N=4 本地实测生成、临时目录清理无残留、真实机 seek 单帧 0.22s/0.34s、真实机 play-info 返回 `thumbVtt` + 两接口 404（生成前）→ chmod 后 seek 抽帧成功。模板 `tool-detect.js` 已同步（8526c72）。**通用 HTML 局部刷新模块（2026-09-21）**——新增模板框架层 `partial-refresh.js`（`server/public/`，零依赖）：`patchList(listEl, items, {key, createRow, renderRow, removeStale})` 按 `data-<key>-id` 复用行 + 增量更新 + 差集删除，不整表 innerHTML 重建（行节点不销毁重建 → 图片不闪、滚动不跳、运行行测速状态不丢）；`patchText`/`patchAttr` 单点原地更新（未变不动，避免无谓 DOM 写）；key 特殊字符走 `CSS.escape` 兜底。下载列表 `renderTask` 接入：`ensureTaskRow` 收敛为纯骨架创建回调，行复用/查行/差集删除统一交给 `patchList`（`data-task-id`），`partial-refresh.js` 加载失败时退回原手写增量逻辑。`scripts.html` 在 app.js 前引入模块；assemble 补 `server/public/partial-refresh.js` 条目。验证：patchList 语义 13/13（行复用不重建/差集删除/新增/特殊字符/未变节点不动）、renderTask 接入 12/12（两次渲染行复用/进度更新/删除/兜底路径）。模板 `_iwara-style/public/app.js` + `fragments/scripts.html` + `server/public/partial-refresh.js` 已同步。**静态资源版本化收敛（2026-09-21）**——此前装配链路存在两套版本号：fragment-assembler 按内容 md5 给 `.js` 加 `?v=`（可靠），但项目 `server/app.js` 的 `injectAssetVersion` 又用 `mtimeMs|0` 覆盖（32 位整数，毫秒精度；git checkout 批量落盘时多文件 mtime 相同/相近 → 版本号不变 → 浏览器命中 `max-age=3600` 缓存继续跑旧 JS，页面改动要手动强刷才生效，且 CSS 版本号只依赖这套劣化逻辑）。最干净做法：**删除 `injectAssetVersion`（项目层），fragment-assembler 的 `versionizeScripts` 扩展为同时覆盖 `<link href="*.css">` 与 `<script src="*.js">`（统一 content-md5 前 8 位）**——版本化收敛到模板层唯一机制，内容一变 URL 必变，批量部署/毫秒级落盘也不会失效；正则用 `[^<]*` 限定标签内 href/src，不误伤 `<a href>`。验证：js/css 版本号均为 md5 hex（6 个资源全覆盖）、改 app.js/style.css 内容 hash 必变、还原即复原、a 标签油猴脚本 `?v=7.7.0` 不被篡改。模板 `fragment-assembler.js` 已同步。**画面水平拖动改为 scrub 实时 seek（2026-09-21）**——`initDragSeek` 重写：拖动画面时**进度条/时间实时跟随手指、画面实时 seek 到目标帧**（不再是松手才跳）；目标时间取整到**整秒**（1s 最小单位），同一秒内微动不重复 seek；播放中拖动先暂停（避免连续 seek 触发缓冲抖动），松手恢复播放（原本在播才恢复）；方向判定沿用 12px 阈值 `|dx|>|dy|` 才进入 seek（竖滑音量不受影响）；`touch-action:none` + pointer 捕获保证触摸屏持续派发、拖出画面不断。验证：jsdom scrub 拖动 9/9（拖动中实时 seek 35s 跟随、取整到秒、播放中暂停、松手恢复播放+定格、同一秒微动不重复 seek、暂停态保持暂停）。模板 `_iwara-style/public/play-enhance.js` 已同步|
