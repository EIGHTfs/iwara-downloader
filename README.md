...
| 版本 | 内容 |
|---|---|
| 1.7.13（未升版） | **播放页点赞/关注按钮补齐取消能力（toggle）+ 措辞「收藏→点赞」+ 搜索列表已赞改 json 字段驱动 + mtime 局部动态刷新**：①播放页——后端新增 `unfollowUser`（`DELETE /user/{id}/followers`，照 `unlikeVideo` 封装姿势）+ `like-state.markUnfollowed` + 路由 `DELETE /api/follow?userId=`；前端 `bindStateButtons` 改 toggle（按 `btn.dataset.on` 判断 DELETE/POST，成功才翻转），标签「❤️ 已点赞」、初始态「❤️ 点赞」（统一改「点赞」措辞）。②搜索列表——badge 渲染改**只读搜索 json 的 `liked` 字段**（单一数据源，服务端 `mergeLikedState` 已补真实值，`likedMeta` 内存兜底退出搜索行渲染）；`/api/liked-state` 返回 `mtime`（liked_state.json 的 stat.mtimeMs）；轮询（搜结果 1.5s / 下载任务 1.5s）改为 **mtime 变化才触发局部更新**：`updateLikedBadges()` 只增删变化行的 `data-liked-badge` span（`data-vid` 定位），**不整表重建**——滚动位置不跳、图片不闪；结构变化（条数/分页）仍走整表渲染。③`assemble.json` 补三条下发条目：`_iwara-style/server/lib/iwara-api.js`、`like-state.js`、`server/routes/like.js` 进模板（后端配套随风格包下发）；`/api/search/cache` 返回前也补 merge。验证：搜索 task.results 实测每条含 liked（96 条 True/False 混合）；`/api/liked-state` 含 mtime（文件 stat 实时）；jsdom 局部更新 8/8（badge 增删/整表不重建/开关/空列表）；播放页 toggle jsdom 9/9（POST/DELETE 分派/翻转/拦截）；真实环境官方 `POST/DELETE /user/{id}/followers` 200。模板 `play-app.js`/`play.html`/`app.js`/`server/lib/*`/`server/routes/like.js` 已同步 |
