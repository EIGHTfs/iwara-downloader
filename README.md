...
| 版本 | 内容 |
|---|---|
| 1.7.13（未升版） | **播放页交互（①点赞/关注 toggle ②搜索列表已赞 json 字段 + mtime 局部刷新 ③画面竖滑音量）**——①播放页点赞/关注按钮补齐取消能力（toggle）+ 措辞「收藏→点赞」：后端新增 `unfollowUser`（`DELETE /user/{id}/followers`）+ `like-state.markUnfollowed` + 路由 `DELETE /api/follow?userId=`；前端 `bindStateButtons` 改 toggle（按 `btn.dataset.on` 判断 DELETE/POST，成功才翻转）。②搜索列表——badge 渲染改**只读搜索 json 的 `liked` 字段**（单一数据源）；`/api/liked-state` 返回 `mtime`（liked_state.json stat.mtimeMs）；轮询改 **mtime 变化才局部更新**：`updateLikedBadges()` 只增删变化行的 badge（`data-vid` 定位），**不整表重建**（滚动不跳、图片不闪）；结构变化（分页）才整表渲染；`/api/search/cache` 返回前也补 merge。③播放器画面手势——上下滑动实时音量（方向分发：第一个位移超 12px 判定横/竖并锁定；竖滑调 `art.volume` + HUD「音量 N%」、满半屏滑满、钳制 0-1、静音自动解除、控制条区域不触发；桌面 pointer 与移动端 touch 都启用，`touch-action:none` 防滚动接管；与 600ms 长按快进互斥——过阈值即 clearHold + 取消点击即播）；左右滑进度仍走原有 seek（连续）。④`assemble.json` 补三条下发条目：`_iwara-style/server/lib/iwara-api.js`、`like-state.js`、`server/routes/like.js` 进模板。验证：jsdom 局部 badge 更新 8/8、竖滑音量 10/10（含 seek 回归/控制条排除/钳制）、播放页 toggle 9/9；真实环境搜索 task.results 每条含 liked（96 条）、`/api/liked-state` 含 mtime、官方 `POST/DELETE /user/{id}/followers` 200。模板 `play-app.js`/`play.html`/`app.js`/`play-enhance.js`/`server/lib/*`/`server/routes/like.js` 已同步 |
